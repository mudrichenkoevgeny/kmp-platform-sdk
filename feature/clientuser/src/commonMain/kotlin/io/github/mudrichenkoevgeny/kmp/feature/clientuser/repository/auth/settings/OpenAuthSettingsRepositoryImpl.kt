 ...
 * via [AuthStorage], HTTP via [OpenAuthSettingsApi], and live updates from [WebSocketService] for
...
class OpenAuthSettingsRepositoryImpl(
    ...
    init {
        repositoryScope.launch {
            _settings.value = authStorage.getPublicAuthSettings()
    ...
    override suspend fun getAuthSettings(): AppResult<PublicAuthSettings> {
        ...
        return updateMutex.withLock {
            val cached = _settings.value ?: authStorage.getPublicAuthSettings()
    ...
    private suspend fun applySettingsUpdate(authSettings: PublicAuthSettings) {
        authStorage.updatePublicAuthSettings(authSettings)
