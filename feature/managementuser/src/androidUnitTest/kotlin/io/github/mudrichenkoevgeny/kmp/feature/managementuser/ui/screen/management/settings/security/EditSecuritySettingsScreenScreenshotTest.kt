...
class EditSecuritySettingsScreenScreenshotTest(
    private val stateName: String,
    private val state: EditSecuritySettingsScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = EditSecuritySettingsComponentMock(initialState = state)
        ...
        if (state is EditSecuritySettingsScreenState.Loading) {
            mainClock.autoAdvance = false
            mainClock.advanceTimeBy(FullscreenLoadingConfig.DELAY_MILLIS + 50L)
        }

        onRoot().captureAppScreen(
            testInstance = this@EditSecuritySettingsScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = EditSecuritySettingsPreviewProvider()
