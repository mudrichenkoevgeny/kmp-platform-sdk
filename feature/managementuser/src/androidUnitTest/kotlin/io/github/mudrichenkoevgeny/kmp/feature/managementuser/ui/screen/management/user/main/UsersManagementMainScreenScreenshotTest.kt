...
class UsersManagementMainScreenScreenshotTest(
    private val stateName: String,
    private val state: UsersManagementMainScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = UsersManagementMainComponentMock(initialState = state)
        ...
        if (state is UsersManagementMainScreenState.Loading) {
            mainClock.autoAdvance = false
            mainClock.advanceTimeBy(FullscreenLoadingConfig.DELAY_MILLIS + 50L)
        }

        onRoot().captureAppScreen(
            testInstance = this@UsersManagementMainScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = UsersManagementMainPreviewProvider()
