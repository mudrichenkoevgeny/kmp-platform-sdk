...
class AuditEventListScreenTest {

    @Test
    fun loading_showsIndeterminateProgressAfterDefaultDelay() = runComposeUiTest {
        val component = AuditEventListComponentMock(AuditEventListScreenState.Loading)
        setContent {
            ComponentTestHarness {
                AuditEventListScreen(component)
            }
        }
    ...
    fun content_displaysElements() = runComposeUiTest {
        ...
        val component = AuditEventListComponentMock(
            AuditEventListScreenState.Content(
        ...
        onNodeWithTag(AuditEventListTestTags.TITLE).assertIsDisplayed()
        onNodeWithTag(AuditEventListTestTags.BACK_BUTTON).assertIsDisplayed()
        onNodeWithTag(AuditEventListTestTags.REFRESH_BUTTON).assertIsDisplayed()
        onNodeWithTag(AuditEventListTestTags.EVENT_LIST).assertIsDisplayed()
    }

    @Test
    fun content_clickBack_invokesCallback() = runComposeUiTest {
        val component = AuditEventListComponentMock(AuditEventListScreenState.Content(paging = PaginationState(emptyList(), isInitialLoading = false)))
        setContent {
            ComponentTestHarness {
                AuditEventListScreen(component)
            }
        }
        onNodeWithTag(AuditEventListTestTags.BACK_BUTTON).performClick()
    ...
    fun content_clickRefresh_invokesCallback() = runComposeUiTest {
        ...
        onNodeWithTag(AuditEventListTestTags.REFRESH_BUTTON).performClick()
        assertEquals(EXPECTED_SINGLE_CALLBACK, component.refreshCalls)
    }

    @Test
    fun error_displaysErrorText() = runComposeUiTest {
        val component = AuditEventListComponentMock(AuditEventListScreenState.Error(CommonError.Unknown()))
        setContent {
            ComponentTestHarness {
                AuditEventListScreen(component)
            }
        }
        onNodeWithTag(AuditEventListTestTags.GLOBAL_ERROR_TEXT).assertIsDisplayed()
    }
