...
class AuditEventListScreenScreenshotTest(
    private val stateName: String,
    private val state: AuditEventListScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = AuditEventListComponentMock(initialState = state)
        ...
        if (state is AuditEventListScreenState.Loading) {
            mainClock.autoAdvance = false
            mainClock.advanceTimeBy(FullscreenLoadingConfig.DELAY_MILLIS + 50L)
        }

        onRoot().captureAppScreen(
            testInstance = this@AuditEventListScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = AuditEventListPreviewProvider()
