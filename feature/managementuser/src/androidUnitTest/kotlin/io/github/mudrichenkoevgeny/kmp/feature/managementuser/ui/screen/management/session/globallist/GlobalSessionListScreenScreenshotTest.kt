...
class GlobalSessionListScreenScreenshotTest(
    private val stateName: String,
    private val state: GlobalSessionListScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = GlobalSessionListComponentMock(initialState = state)
        ...
        if (state is GlobalSessionListScreenState.Loading ||
            (state is GlobalSessionListScreenState.Content && state.paging.isInitialLoading && state.paging.items.isEmpty())
        ) {
        ...
        onRoot().captureAppScreen(
            testInstance = this@GlobalSessionListScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = GlobalSessionListPreviewProvider()
