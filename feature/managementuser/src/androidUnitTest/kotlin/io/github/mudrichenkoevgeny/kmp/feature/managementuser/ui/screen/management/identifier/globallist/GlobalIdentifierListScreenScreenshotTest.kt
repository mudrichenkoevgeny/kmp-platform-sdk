...
class GlobalIdentifierListScreenScreenshotTest(
    private val stateName: String,
    private val state: GlobalIdentifierListScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = GlobalIdentifierListComponentMock(initialState = state)
        ...
        if (state is GlobalIdentifierListScreenState.Loading) {
            mainClock.autoAdvance = false
            mainClock.advanceTimeBy(FullscreenLoadingConfig.DELAY_MILLIS + 50L)
        }

        onRoot().captureAppScreen(
            testInstance = this@GlobalIdentifierListScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = GlobalIdentifierListPreviewProvider()
