...
class UserIdentifiersScreenScreenshotTest(
    private val stateName: String,
    private val state: UserIdentifierListScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = UserIdentifierListComponentMock(initialState = state)
        ...
        if (state is UserIdentifierListScreenState.Loading) {
            mainClock.autoAdvance = false
            mainClock.advanceTimeBy(FullscreenLoadingConfig.DELAY_MILLIS + 50L)
        }

        onRoot().captureAppScreen(
            testInstance = this@UserIdentifiersScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = UserIdentifierListPreviewProvider()
