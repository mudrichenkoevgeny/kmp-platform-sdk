...
class UserSessionsScreenScreenshotTest(
    private val stateName: String,
    private val state: UserSessionsScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = UserSessionsComponentMock(initialState = state)
        ...
        if (state is UserSessionsScreenState.Loading) {
            mainClock.autoAdvance = false
            mainClock.advanceTimeBy(FullscreenLoadingConfig.DELAY_MILLIS + 50L)
        }

        onRoot().captureAppScreen(
            testInstance = this@UserSessionsScreenScreenshotTest,
            stateName = stateName
        )
    }
