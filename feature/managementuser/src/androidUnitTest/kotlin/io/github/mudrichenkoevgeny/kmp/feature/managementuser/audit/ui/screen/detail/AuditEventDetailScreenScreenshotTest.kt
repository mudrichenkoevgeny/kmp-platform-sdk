...
class AuditEventDetailScreenScreenshotTest(
    private val stateName: String,
    private val state: AuditEventDetailScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = AuditEventDetailComponentMock(initialState = state)
        ...
        if (state is AuditEventDetailScreenState.Loading) {
            mainClock.autoAdvance = false
            mainClock.advanceTimeBy(FullscreenLoadingConfig.DELAY_MILLIS + 50L)
        }

        onRoot().captureAppScreen(
            testInstance = this@AuditEventDetailScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = AuditEventDetailPreviewProvider()
