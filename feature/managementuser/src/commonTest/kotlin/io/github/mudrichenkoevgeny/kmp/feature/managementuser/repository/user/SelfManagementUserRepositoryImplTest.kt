...
import io.github.mudrichenkoevgeny.kmp.feature.managementuser.mock.network.api.user.SelfManagementUserApiMock
...
@InternalApi
class SelfManagementUserRepositoryImplTest {

    private val userStorage = UserStorageMock()
    private val authStorage = AuthStorageMock()
    private val userApi = SelfManagementUserApiMock()
    private val webSocketService = WebSocketServiceMock()

    private fun createRepository(scope: TestScope): SelfManagementUserRepositoryImpl {
        return SelfManagementUserRepositoryImpl(
            userStorage = userStorage,
            authStorage = authStorage,
            selfManagementUserApi = userApi,
    ...
    @Test
    fun `scheduleUserDeletion returns error as unsupported`() = runTest {
        val repo = createRepository(this)
    ...
    @Test
    fun `restoreUser returns error as unsupported`() = runTest {
        val repo = createRepository(this)
