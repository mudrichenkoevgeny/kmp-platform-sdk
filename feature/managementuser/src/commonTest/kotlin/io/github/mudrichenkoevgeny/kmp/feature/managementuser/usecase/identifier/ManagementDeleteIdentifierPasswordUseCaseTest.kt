...
class ManagementDeleteIdentifierPasswordUseCaseTest {
    ...
    fun invoke_returnsSuccess_whenRepositorySucceeds() = runTest {
        val repository = ManagementIdentifierRepositoryMock().apply {
            deleteIdentifierPasswordResultProvider = { _, _ -> AppResult.Success(Unit) }
        }
        val useCase = ManagementDeleteIdentifierPasswordUseCase(repository)
    ...
    fun invoke_returnsError_whenRepositoryFails() = runTest {
        ...
        val repository = ManagementIdentifierRepositoryMock().apply {
            deleteIdentifierPasswordResultProvider = { _, _ -> AppResult.Error(expectedError) }
        }
        val useCase = ManagementDeleteIdentifierPasswordUseCase(repository)
        val userId = UserId.generate()
        val identifierId = "identifier_id"
