    ...
    Scaffold(
            ...
            TopAppBar(
                ...
                navigationIcon = {
                    CoreBackButton(
                        ...
                        modifier = Modifier.testTag(GlobalSessionListTestTags.BACK_BUTTON)
                    )
                },
                actions = {
                    IconButton(
                        onClick = component::onToggleFilterPanel,
                        modifier = Modifier.testTag(GlobalSessionListTestTags.FILTER_BUTTON)
                    ) {
                        Icon(
                    ...
                    IconButton(
                        ...
                        modifier = Modifier.testTag(GlobalSessionListTestTags.REFRESH_BUTTON)
                    ) {
                        Icon(
        ...
        Box(
            ...
            when (val currentState = state) {
                is GlobalSessionListScreenState.Loading -> FullscreenLoading()
                is GlobalSessionListScreenState.Content -> {
                ...
                is GlobalSessionListScreenState.Error -> {
                    CoreErrorText(
                        text = currentState.error.toLocalizedMessage(),
                        modifier = Modifier.testTag(GlobalSessionListTestTags.GLOBAL_ERROR_TEXT)
                    )
                }
            }
        }
...
@Composable
private fun Content(
    state: GlobalSessionListScreenState.Content,
    component: GlobalSessionListComponent,
    onLoadNextPage: () -> Unit
) {
    ...
    Box(
        ...
        Column(
            ...
            AnimatedVisibility(visible = state.isFilterPanelExpanded) {
                ListingOptionsPanel(
                    config = getGlobalSessionListingOptionsConfig(),
            ...
            state.actionError?.let {
                CoreErrorText(
                    ...
                    modifier = Modifier
                        .fillMaxWidth()
                        .padding(CoreTheme.dimens.paddingMedium)
                        .testTag(GlobalSessionListTestTags.ACTION_ERROR_TEXT)
                )
            }
...
@InternalApi
internal class GlobalSessionListPreviewProvider : PreviewParameterProvider<GlobalSessionListScreenState> {
    private val items: List<Pair<String, GlobalSessionListScreenState>> = listOf(
        "Content" to GlobalSessionListScreenState.Content(
            paging = PaginationState(
                items = listOf(userSessionMock(), userSessionMock())
            )
        ),
        "Error" to GlobalSessionListScreenState.Error(error = CommonError.Unknown()),
        "Loading" to GlobalSessionListScreenState.Loading
    )

    override val values: Sequence<GlobalSessionListScreenState> = items.asSequence().map { it.second }
...
@InternalApi
@Composable
private fun GlobalSessionListScreenPreviewContent(state: GlobalSessionListScreenState) {
    CompositionLocalProvider(LocalErrorParser provides AppErrorParserMock) {
        GlobalSessionListScreen(
            component = GlobalSessionListComponentMock(initialState = state)
        )
    }
}
...
private fun StatesPreviewGlobal(
    ...
    ScreenPreviewContainer {
        GlobalSessionListScreenPreviewContent(state = state)
    }
}
...
object GlobalSessionListTestTags {
    const val TITLE = "GlobalSessionList_Title"
    const val BACK_BUTTON = "GlobalSessionList_BackButton"
