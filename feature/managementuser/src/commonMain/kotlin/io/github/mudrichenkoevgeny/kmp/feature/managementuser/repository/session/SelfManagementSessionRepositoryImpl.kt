 ...
 * Falls back to [UserStorage] cache with client-side filtering and pagination if network calls fail.
 *
...
class SelfManagementSessionRepositoryImpl(
    ...
    override suspend fun getSessions(
        ...
        val networkResult = selfManagementSessionApi.getSessions(
    ...
    override suspend fun getSession(userSessionId: UserSessionId): AppResult<UserSession> {
        val networkResult = selfManagementSessionApi.getSession(userSessionId)
        return when (networkResult) {
            is AppResult.Success -> {
    ...
    override suspend fun deleteSession(userSessionId: UserSessionId): AppResult<Unit> {
        val networkResult = selfManagementSessionApi.deleteSession(userSessionId)
        if (networkResult is AppResult.Success) {
    ...
    override suspend fun deleteAllOtherSessions(): AppResult<Unit> {
        val networkResult = selfManagementSessionApi.deleteAllOtherSessions()
        if (networkResult is AppResult.Success) {
    ...
    override suspend fun reauthenticateSession(mfaToken: String, code: String): AppResult<Unit> {
        return selfManagementSessionApi.reauthenticateSession(
            VerifyTotpPayload(
                mfaToken = mfaToken,
                code = code
            )
