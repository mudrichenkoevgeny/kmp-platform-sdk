...
class RegenerateRecoveryCodesUseCaseTest {
    ...
    @Test
    fun invoke_returnsNewRecoveryCodes_whenRepositorySucceeds() = runTest {
        val expectedCodes = totpRecoveryCodesMock()
        val repository = UserSecurityRepositoryMock().apply {
            regenerateRecoveryCodesResultProvider = { AppResult.Success(expectedCodes) }
        }
        val useCase = RegenerateRecoveryCodesUseCase(repository)
    ...
    fun invoke_returnsError_whenRepositoryFails() = runTest {
        ...
        val repository = UserSecurityRepositoryMock().apply {
            regenerateRecoveryCodesResultProvider = { AppResult.Error(expectedError) }
        }
        val useCase = RegenerateRecoveryCodesUseCase(repository)
