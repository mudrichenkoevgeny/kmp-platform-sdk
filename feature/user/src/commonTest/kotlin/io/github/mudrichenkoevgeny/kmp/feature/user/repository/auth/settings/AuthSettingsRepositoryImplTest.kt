...
import io.github.mudrichenkoevgeny.kmp.feature.user.mock.network.api.auth.settings.AuthSettingsApiMock
...
class AuthSettingsRepositoryImplTest {
    ...
    private val api = AuthSettingsApiMock()
    private val sockets = WebSocketServiceMock()

    private fun createRepository(scope: TestScope) = AuthSettingsRepositoryImpl(
        authSettingsApi = api,
        authStorage = storage,
    ...
    fun refreshAuthSettings_alwaysCallsApi() = runTest {
        storage.updateAuthSettings(publicAuthSettingsPayloadMock().toAuthSettings())
