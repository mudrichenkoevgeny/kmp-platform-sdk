...
class MainProfileScreenScreenshotTest(
    private val stateName: String,
    private val state: MainProfileScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = MainProfileComponentMock(initialState = state)
        ...
        if ((state is MainProfileScreenState.Loading) || (state is MainProfileScreenState.Content && state.actionLoading)) {
            mainClock.autoAdvance = false
            mainClock.advanceTimeBy(FullscreenLoadingConfig.DELAY_MILLIS + 50L)
        }

        onRoot().captureAppScreen(
            testInstance = this@MainProfileScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = MainProfilePreviewProvider()
