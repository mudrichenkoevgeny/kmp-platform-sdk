...
class LoginByTotpScreenScreenshotTest(
    private val stateName: String,
    private val state: LoginByTotpScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = LoginByTotpComponentMock(initialState = state)
        ...
        if ((state is LoginByTotpScreenState.Loading) || (state is LoginByTotpScreenState.Content && state.actionLoading)) {
            mainClock.autoAdvance = false
            mainClock.advanceTimeBy(FullscreenLoadingConfig.DELAY_MILLIS + 50L)
        }

        onRoot().captureAppScreen(
            testInstance = this@LoginByTotpScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = LoginByTotpPreviewProvider()
