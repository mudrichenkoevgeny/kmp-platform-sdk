...
import io.github.mudrichenkoevgeny.kmp.feature.user.ui.screen.profile.totp.main.TotpMainPreviewProvider
...
class TotpMainScreenScreenshotTest(
    private val stateName: String,
    private val state: TotpMainScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = TotpMainComponentMock(initialState = state)
        ...
        if ((state is TotpMainScreenState.Loading) ||
            (state is TotpMainScreenState.Disabled && state.actionLoading) ||
            (state is TotpMainScreenState.SetupInProgress && state.actionLoading) ||
            (state is TotpMainScreenState.Enabled && state.actionLoading)
        ) {
        ...
        onRoot().captureAppScreen(
            testInstance = this@TotpMainScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = TotpMainPreviewProvider()
