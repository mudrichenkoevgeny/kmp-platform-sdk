...
class LoginByEmailScreenScreenshotTest(
    private val stateName: String,
    private val state: LoginByEmailScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = LoginByEmailComponentMock(initialState = state)
        ...
        if ((state is LoginByEmailScreenState.Loading) || (state is LoginByEmailScreenState.Content && state.actionLoading)) {
            mainClock.autoAdvance = false
            mainClock.advanceTimeBy(FullscreenLoadingConfig.DELAY_MILLIS + 50L)
        }

        onRoot().captureAppScreen(
            testInstance = this@LoginByEmailScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = LoginByEmailPreviewProvider()
