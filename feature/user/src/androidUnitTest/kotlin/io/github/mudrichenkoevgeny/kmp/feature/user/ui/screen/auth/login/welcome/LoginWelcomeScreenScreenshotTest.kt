...
class LoginWelcomeScreenScreenshotTest(
    private val stateName: String,
    private val state: LoginWelcomeScreenState
) {

    @Test
    fun capture() = runComposeUiTest {
        val component = LoginWelcomeComponentMock(initialState = state)
        ...
        if ((state is LoginWelcomeScreenState.Loading) || (state is LoginWelcomeScreenState.Content && state.actionLoading)) {
            mainClock.autoAdvance = false
            mainClock.advanceTimeBy(FullscreenLoadingConfig.DELAY_MILLIS + 50L)
        }

        onRoot().captureAppScreen(
            testInstance = this@LoginWelcomeScreenScreenshotTest,
            stateName = stateName
        )
    }
    ...
    companion object {
        ...
        fun data(): Collection<Array<Any>> {
            val provider = LoginWelcomePreviewProvider()
