...
/**
 * Default [SelfIdentifierListComponent] implementation: manages account identifiers (email, phone).
 *
...
class SelfIdentifierListComponentImpl(
            ...
            sendAddEmailIdentifierConfirmationUseCase(email)
                .onSuccess {
                    _state.value = current.copy(
                        actionLoading = false,
                        addEmailState = SelfIdentifierListScreenState.AddIdentifierState.EnteringCode(value = email)
                    )
                }
                .onError { error ->
                    _state.value = current.copy(actionLoading = false, actionError = error)
    ...
    override fun onEmailCodeChanged(code: String) {
        val current = _state.value as? SelfIdentifierListScreenState.Content ?: return
        val addState = current.addEmailState as? SelfIdentifierListScreenState.AddIdentifierState.EnteringCode ?: return
        ...
        scope.launch {
            sendAddPhoneIdentifierConfirmationUseCase(phoneNumber)
                .onSuccess {
                    _state.value = current.copy(
                        actionLoading = false,
                        addPhoneState = SelfIdentifierListScreenState.AddIdentifierState.EnteringCode(value = phoneNumber)
                    )
                }
                .onError { error ->
                    _state.value = current.copy(actionLoading = false, actionError = error)
    ...
    override fun onPhoneCodeChanged(code: String) {
        val current = _state.value as? SelfIdentifierListScreenState.Content ?: return
        val addState = current.addPhoneState as? SelfIdentifierListScreenState.AddIdentifierState.EnteringCode ?: return
    ...
    override fun onCancelAddClick() {
        val current = _state.value as? SelfIdentifierListScreenState.Content ?: return
        _state.value = current.copy(
            addEmailState = SelfIdentifierListScreenState.AddIdentifierState.Idle,
        ...
        scope.launch {
            getUserIdentifiersUseCase(
                ...
                .onSuccess { pagedResult ->
                    ...
                    _state.value = SelfIdentifierListScreenState.Content(paging = newPaging)
                }
                .onError { error ->
                    val currentContent = _state.value as? SelfIdentifierListScreenState.Content
                    if (currentContent != null) {
